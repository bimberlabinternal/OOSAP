...
PlotImmuneMarkers <- function(seuratObj, reductions = c('tsne', 'umap')) {
  ...
  #PTPRC = CD45
  #SELL = CD62-L / CD-197
  PlotMarkerSeries(seuratObj, reductions, c('CCR7', 'SELL', 'GZMB', 'CCR5', 'IL2RA', 'PTPRC', 'IL7R', 'CTLA4'), 'Effector vs. Memory')
  ...
  PlotMarkerSet(seuratObj, reductions, 'Transcription Factors', c('TBX21', 'GATA3', 'RORC', 'FOXP3', 'BCL6', 'EOMES', 'TOX'))
  ...
  #LILR/KIR:
  PlotMarkerSeries(seuratObj, reductions, c('LILRA5','LILRA6','LILRB4','LILRB5','KIR2DL4','KIR3DX1', 'MAMU-KIR', 'KIR2DL4', 'KIR3DL2'), 'KIR/LILR')

  PlotMarkerSeries(seuratObj, reductions, c('FCGR1A','FCGR2B','FCGR3'), 'FCGR')
...
PlotMarkerSet <- function(seuratObj, reductions, title, features) {
	...
	for (reduction in reductions) {
		...
		plotList[i] <- AddTitleToMultiPlot(FeaturePlot(seuratObj, features = featuresToPlot, reduction = reduction, min.cutoff = 'q05', max.cutoff = 'q95'), title)
	}

  print(patchwork::wrap_plots(plotList, ncol = length(reductions)))
}
