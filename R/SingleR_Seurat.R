...
DimPlot_SingleRClassLabs <- function(seuratObject, plotIndividually = F){
    plots <- list(
      DimPlot(seuratObject, group.by = "SingleR_Labels") + theme_bw() + ggtitle("SingleR Labels") + theme(legend.position="bottom"),
      DimPlot(seuratObject, group.by = "SingleR_Labels_Fine") + theme_bw() + ggtitle("SingleR Labels (Fine)") + theme(legend.position="bottom")
    )
