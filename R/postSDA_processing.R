...
RecomposeSDAmatrix <- function(SDAseuratObj = NULL, compRemoveSet = "batch",  
                               metaDF = NULL, projectName = 'imputedSeuratObj',
                                spikeGenes = NULL, nVariableFeatures = 2000,  variableFeatureSelectionMethod = "vst",
                                tSNE_perplexity = 30, tSNE_maxIter = 2000,
                                UMAP_NumNeib = 40L, UMAP_MinDist = 0.2, UMAP_Seed = 1234, UMAP.NumEpoc = 500
) 
{
  ...
  if(compRemoveSet == "none") {
    compKeepSet <- SDAseuratObj@misc$SDA_processing_results$QC_components
  } else {
    if(compRemoveSet == "qc"){
      compKeepSet <- setdiff(1:ncol(SDAseuratObj@reductions$SDA@cell.embeddings), as.numeric(SDAseuratObj@misc$SDA_processing_results$QC_components))
    } else {
      if(compRemoveSet == "batch"){
        compKeepSet <- setdiff(1:ncol(SDAseuratObj@reductions$SDA@cell.embeddings), as.numeric(SDAseuratObj@misc$SDA_processing_results$Remove_comps))
      }
    }
  }
  
  print("Performing dot product...")
  recomposedMat <- t(SDAseuratObj@reductions$SDA@cell.embeddings[,compKeepSet] %*% t(SDAseuratObj@reductions$SDA@feature.loadings)[compKeepSet,])
